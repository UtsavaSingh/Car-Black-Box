    ...
    if(i == 4)
    {
        ...
        else
        {
            ...
            if(attempt_rem == 0)
            {
                ...
                clcd_print("wait for 15 min", (0xC0 + 0));
...
void view_log(unsigned char reset_flag, unsigned char key)
{
    ...
    if(pos == -1)
    {
        clcd_print("No Log Exist", (0x80 + 1));
    }
    else
    {
        if(reset_flag == 0x13)
        ...
        if(key == 0x2F && (log_pos < (access-1)))
        {
            log_pos++;
            clear_screen();
        }
        else if(key == 0x37 && log_pos > 0)
        {
            log_pos--;
            clear_screen();
        }
        else if(key == 0x2F && (log_pos == (access-1)))
        {
            log_pos = 0;
            clear_screen();
        }
        if(log_pos < access)
        {
            clcd_print("# TIME     EV SP", (0x80 + 0));
            i = log_pos * 10 + 5;
            clcd_putch(log_pos + '0', (0xC0 + 0));
            ch = read_EEPROM(i);
            clcd_putch(ch, (0xC0 + 2));
            ch = read_EEPROM(i+1);
            clcd_putch(ch, (0xC0 + 3));
            clcd_putch(':', (0xC0 + 4));
            ch = read_EEPROM(i+2);
            clcd_putch(ch, (0xC0 + 5));
            ch = read_EEPROM(i+3);
            clcd_putch(ch, (0xC0 + 6));
            clcd_putch(':', (0xC0 + 7));
            ch = read_EEPROM(i+4);
            clcd_putch(ch, (0xC0 + 8));
            ch = read_EEPROM(i+5);
            clcd_putch(ch, (0xC0 + 9));
            clcd_putch(' ', (0xC0 + 10));
            ch = read_EEPROM(i+6);
            clcd_putch(ch, (0xC0 + 11));
            ch = read_EEPROM(i+7);
            clcd_putch(ch, (0xC0 + 12));
            clcd_putch(' ', (0xC0 + 13));
            ch = read_EEPROM(i+8);
            clcd_putch(ch, (0xC0 + 14));
            ch = read_EEPROM(i+9);
            clcd_putch(ch, (0xC0 + 15));
        }
    }
}
    ...
    if(i == 4)
    {
        ...
        if(j == 4)
        {
            if(strncmp(npasswd, re_npasswd, 4) == 0)
            {
                ...
                clcd_write(0x0C, 0);
                _delay((unsigned long)((100)*(20000000/4000000.0)));
                clcd_print("New Password set", (0x80 + 0));
            ...
            else
            {
                ...
                clcd_write(0x0C, 0);
                _delay((unsigned long)((100)*(20000000/4000000.0)));
                clcd_print(" Both Password ", (0x80 + 0));
...
char download_log(unsigned char reset_flag)
{
    ...
    else
    {
        ...
        {
            log_pos = 0;
            clcd_print("Downloaded-Open", (0x80 + 0));
            clcd_print("Serial Terminal", (0xC0 + 0));
            putchar('\n');
            puts("N TIME     EV SP\n");
        }
